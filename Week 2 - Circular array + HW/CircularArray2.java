public class CircularArray2{
   ...
   */
  public CircularArray2(Object [] lin, int st, int sz){
...
// Start index becomes zero
  ...
  public void insertByRightShift(Object elem, int pos){
    ...
    int lastavlpos=start+size;
    int lastelem=start+size-1;
    for(int i=0;i<size-pos;i++){
      cir[lastavlpos%cir.length]=cir[lastelem%cir.length];
      lastavlpos--;
      lastelem--;
      if(lastavlpos==-1){
        lastavlpos=cir.length-1;
      }
      if(lastelem==-1){
        lastelem=cir.length-1;
      }
    }
...
//This method will sort the values by keeping the start unchanged
  public Object sort(){
    ...
    return cir[(start+1)%cir.length];
  }
  
//This method will check the given array across the base array and if they are equivalent interms of values return true, or else return false
  public boolean equivalent(CircularArray2 k){
    boolean result=true;
    for(int i=0;i<size;i++){
